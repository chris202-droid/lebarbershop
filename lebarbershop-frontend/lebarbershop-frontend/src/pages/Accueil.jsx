...
/**
 * Motif décoratif discret (points + traits organiques verts/jaunes) posé en
 * arrière-plan d'une section. Toujours en position absolute, très faible
 * opacité et pointer-events désactivés : jamais au-dessus du contenu ni au
 * détriment de la lisibilité — juste une texture de marque en filigrane.
 */
function MotifDecoratif({ position = "right", teinte = "vert" }) {
  const couleur = teinte === "jaune" ? T.gold : T.mint;
  const cote = position === "right" ? "right-0" : "left-0";
  return (
    <svg
      className={`pointer-events-none absolute top-0 ${cote} w-[420px] h-[420px] -z-10 opacity-[0.06]`}
      viewBox="0 0 420 420" fill="none" aria-hidden="true"
    >
      <circle cx="330" cy="70" r="140" stroke={couleur} strokeWidth="1.5" />
      <circle cx="330" cy="70" r="90" stroke={couleur} strokeWidth="1.5" />
      <path d="M40 380 C 140 300, 220 340, 360 220" stroke={couleur} strokeWidth="1.5" />
      {Array.from({ length: 24 }).map((_, i) => (
        <circle key={i} cx={40 + (i % 6) * 26} cy={260 + Math.floor(i / 6) * 26} r="2.5" fill={couleur} />
      ))}
    </svg>
  );
}
...
/**
 * Rubrique de la landing page : révélation en fondu + léger glissement vers
 * le haut au moment où elle entre dans le viewport (point demandé :
 * "animations de rubrique sur la page d'accueil"), sobre et une seule fois
 * par section pour rester professionnel.
 */
function Section({ id, eyebrow, titre, sous_titre, children, motif }) {
  const [ref, enVue] = useEnVue();
  return (
    <section id={id} ref={ref} className="relative px-6 py-16 max-w-6xl mx-auto scroll-mt-20 overflow-hidden"
      style={{
        opacity: enVue ? 1 : 0,
        transform: enVue ? "translateY(0)" : "translateY(24px)",
        transition: "opacity 0.7s ease-out, transform 0.7s ease-out",
      }}>
      {motif && <MotifDecoratif position={motif} teinte={motif === "left" ? "jaune" : "vert"} />}
      {eyebrow && (
...
export default function Accueil() {
  ...
  const [heroMonte, setHeroMonte] = useState(false);

  // Animation d'entrée du Hero au premier affichage de la page (léger
  // décalage pour laisser le fondu se percevoir plutôt qu'un flash instantané).
  useEffect(() => {
    const id = requestAnimationFrame(() => setHeroMonte(true));
  ...
  const soumettrePromo = async (e) => {
    ...
    try {
      ...
      setCodePromo(resultat); // { code, montant_reduction, date_expiration }
    ...
    <div className="w-full min-h-screen" style={{ background: T.ink, fontFamily: "Manrope, sans-serif" }}>
      ...
      <section className="relative overflow-hidden px-6 pt-16 pb-20">
        ...
        <div className="relative max-w-3xl mx-auto text-center"
          style={{
            opacity: heroMonte ? 1 : 0,
            transform: heroMonte ? "translateY(0)" : "translateY(16px)",
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 mt-8">
            <Link to="/inscription" className="flex items-center gap-2 px-5 py-3 rounded-md text-sm font-semibold" style={{ background: T.mint, color: T.inkDeep }}>
      ...
      {/* Qui sommes-nous */}
      <Section id="qui-sommes-nous" motif="right" eyebrow="Qui sommes-nous" titre="Une plateforme pensée pour les salons camerounais"
      ...
      {/* Nos forfaits */}
      <Section id="forfaits" motif="left" eyebrow="Nos forfaits" titre="Des tarifs simples, pensés pour démarrer vite"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-6">
          <div className="rounded-lg p-6 relative overflow-hidden" style={{ background: T.inkDeep, border: `2px solid ${T.gold}` }}>
            <span className="absolute top-4 right-4 text-[10px] uppercase tracking-widest px-2 py-1 rounded-full" style={{ background: "rgba(201,147,42,0.15)", color: T.gold }}>Tarif de lancement</span>
            <p style={{ fontFamily: "Fraunces, serif", fontSize: 20, color: T.clair }}>Premier abonnement salon</p>
            <p className="mt-3"><span className="font-mono text-3xl" style={{ color: T.gold }}>1 500</span> <span className="text-sm" style={{ color: "rgba(245,241,232,0.6)" }}>FCFA / mois</span></p>
            <p className="text-xs mt-1" style={{ color: "rgba(245,241,232,0.6)" }}>Minimum 3 mois, soit 4 500 FCFA</p>
            ...
            <Link to="/inscription" className="mt-6 flex items-center justify-center gap-2 px-4 py-2.5 rounded-md text-sm font-semibold" style={{ background: T.mint, color: T.inkDeep }}>
          ...
          <div className="rounded-lg p-6" style={{ background: "rgba(18,42,32,0.04)", border: `1px solid ${T.line}` }}>
            ...
            <div className="space-y-2 mt-5">
              {["Toutes les fonctionnalités incluses", "Paiement Orange Money, MTN MoMo, carte", "Support et mises à jour continues"].map((a) => (
                <div key={a} className="flex items-center gap-2 text-sm" style={{ color: "rgba(18,42,32,0.75)" }}><CheckCircle2 size={14} style={{ color: T.mint }} /> {a}</div>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4 mt-8 pt-6" style={{ borderTop: `1px dashed ${T.line}` }}>
          <span className="text-xs" style={{ color: "rgba(18,42,32,0.45)" }}>Moyens de paiement acceptés :</span>
          {[["Orange Money", Smartphone], ["MTN Mobile Money", Smartphone], ["Carte Visa / Mastercard", CardIcon]].map(([l, Icon]) => (
            <span key={l} className="flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full" style={{ background: "rgba(18,42,32,0.05)", color: "rgba(18,42,32,0.65)" }}>
              <Icon size={12} /> {l}
            </span>
          ))}
        </div>
      ...
      {/* Devenir partenaire */}
      <Section id="partenaire" motif="right" eyebrow="Devenir partenaire" titre="Recevez un code de sponsoring"
            ...
            {[
              ...
              <div key={f.titre} className="flex gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs mt-0.5" style={{ color: "rgba(18,42,32,0.5)" }}>{f.texte}</p>
                </div>
              </div>
            ))}
          </div>

          <form onSubmit={soumettrePartenaire} className="rounded-lg p-6 space-y-3" style={{ background: "rgba(18,42,32,0.04)", border: `1px solid ${T.line}` }}>
            {envoye.partenaire ? (
              ...
              <>
                ...
                <input required value={formPartenaire.nom} onChange={(e) => setFormPartenaire({ ...formPartenaire, nom: e.target.value })}
                  placeholder="Votre nom" className="w-full px-3 py-2.5 rounded-md text-sm outline-none" style={{ background: "rgba(18,42,32,0.05)", color: T.ivory, border: `1px solid ${T.line}` }} />
                ...
                <button type="submit" disabled={envoi.partenaire} className="w-full py-2.5 rounded-md text-sm font-semibold flex items-center justify-center gap-2" style={{ background: T.mint, color: T.inkDeep, opacity: envoi.partenaire ? 0.6 : 1 }}>
      ...
      {/* Code promo */}
      <Section id="code-promo" motif="left" eyebrow="Code promo" titre="Obtenir un code de réduction"
        sous_titre="Laissez votre email pour recevoir un code de réduction sur votre premier abonnement salon.">
        <div className="rounded-lg p-6 max-w-lg" style={{ background: "rgba(18,42,32,0.04)", border: `1px solid ${T.line}` }}>
          {envoye.promo && codePromo ? (
            <div>
              ...
              <div className="flex items-center justify-between px-4 py-3 rounded-md" style={{ background: "rgba(201,147,42,0.1)", border: `1px dashed ${T.gold}` }}>
                <span className="font-mono text-lg tracking-wide" style={{ color: T.gold }}>{codePromo.code}</span>
                <span className="text-xs" style={{ color: "rgba(18,42,32,0.6)" }}>-{Number(codePromo.montant_reduction).toLocaleString()} FCFA</span>
              </div>
              <p className="text-[11px] mt-2" style={{ color: "rgba(18,42,32,0.4)" }}>
            ...
            <form onSubmit={soumettrePromo} className="flex flex-col sm:flex-row gap-2">
              <div className="relative flex-1">
                <Tag size={15} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: "rgba(18,42,32,0.4)" }} />
                <input required type="email" value={formPromo.email} onChange={(e) => setFormPromo({ email: e.target.value })}
                  placeholder="votre@email.com" className="w-full pl-9 pr-3 py-2.5 rounded-md text-sm outline-none" style={{ background: "rgba(18,42,32,0.05)", color: T.ivory, border: `1px solid ${T.line}` }} />
              </div>
              <button type="submit" disabled={envoi.promo} className="px-4 py-2.5 rounded-md text-sm font-semibold flex items-center justify-center gap-2 shrink-0" style={{ background: T.coral, color: T.clair, opacity: envoi.promo ? 0.6 : 1 }}>
          ...
          <p className="text-[11px] mt-3" style={{ color: "rgba(18,42,32,0.4)" }}>
      ...
      {/* Contact */}
      <Section id="contact" motif="right" eyebrow="Nous contacter" titre="Une question ? Écrivez-nous"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          ...
          <form onSubmit={soumettreContact} className="lg:col-span-3 rounded-lg p-6 space-y-3" style={{ background: "rgba(18,42,32,0.04)", border: `1px solid ${T.line}` }}>
            {envoye.contact ? (
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <input required type="email" value={formContact.email} onChange={(e) => setFormContact({ ...formContact, email: e.target.value })}
                    placeholder="Email" className="px-3 py-2.5 rounded-md text-sm outline-none" style={{ background: "rgba(18,42,32,0.05)", color: T.ivory, border: `1px solid ${T.line}` }} />
                </div>
                <select value={formContact.motif} onChange={(e) => setFormContact({ ...formContact, motif: e.target.value })}
                  ...
                  <option value="information" style={{ background: T.inkDeep, color: T.clair }}>Prise d'information</option>
                  <option value="ouverture_salon" style={{ background: T.inkDeep, color: T.clair }}>Ouvrir un salon</option>
                  <option value="partenariat" style={{ background: T.inkDeep, color: T.clair }}>Devenir partenaire</option>
                  <option value="autre" style={{ background: T.inkDeep, color: T.clair }}>Autre</option>
                </select>
                ...
                <button type="submit" disabled={envoi.contact} className="px-5 py-2.5 rounded-md text-sm font-semibold flex items-center gap-2" style={{ background: T.mint, color: T.inkDeep, opacity: envoi.contact ? 0.6 : 1 }}>
