...
function VueCodes() {
  ...
  const [nouveauCodeReduc, setNouveauCodeReduc] = useState({
    code: "", pourcentage_reduction: "", proprietaire_nom: "", proprietaire_contact: "", duree_jours: "30",
  });
  ...
  const ajouterCodeReduction = async (e) => {
    e.preventDefault();
    if (!nouveauCodeReduc.code || !nouveauCodeReduc.pourcentage_reduction) return;
    setErreurCreation(""); setEnvoiCode(true);
    try {
      const c = await creerCodeReduction({
        ...nouveauCodeReduc,
        pourcentage_reduction: Number(nouveauCodeReduc.pourcentage_reduction),
        duree_jours: nouveauCodeReduc.duree_jours ? Number(nouveauCodeReduc.duree_jours) : undefined,
      }); // POST /api/v1/codes-reduction/ — propriétaire/contact/durée/montant (point demandé)
      setCodesReduction([c, ...codesReduction]);
      setNouveauCodeReduc({ code: "", pourcentage_reduction: "", proprietaire_nom: "", proprietaire_contact: "", duree_jours: "30" });
    } catch (err) {
  ...
  const ajouterCodeSponsoring = async (e) => {
    ...
    try {
      const c = await creerCodeSponsoring(nouveauCodeSpons);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
      ...
      <Carte>
        ...
        <form onSubmit={ajouterCodeReduction} className="space-y-2 mb-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <input required value={nouveauCodeReduc.pourcentage_reduction} onChange={(e) => setNouveauCodeReduc({ ...nouveauCodeReduc, pourcentage_reduction: e.target.value })} placeholder="% de réduction" type="number" min="1" max="100"
          ...
          {codesReduction.map((c) => (
            <div key={c.id} className="flex items-center justify-between p-2.5 rounded-md" style={{ background: "rgba(246,239,221,0.03)" }}>
              <div>
                ...
                <span className="font-mono text-xs ml-2" style={{ color: T.gold }}>{c.pourcentage_reduction}%</span>
                {c.proprietaire_nom && (
      ...
      <Carte>
        ...
        <form onSubmit={ajouterCodeSponsoring} className="flex gap-2 mb-3">
